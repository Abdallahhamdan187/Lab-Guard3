...
export function BorrowRequest() {
  ...
  const preId       = location.state?.selectedEquipmentId || "";
  ...
  const myActiveBorrows = mockTransactions.filter(t =>
    t.userName === sessionUser?.name &&
    t.type === "Borrow" &&
    t.status === "Approved" &&
    !t.returnDate
  );

  const selectedEquipment = mockEquipment.find(e => e.id === borrowForm.equipmentId);
  const selectedTxn       = myActiveBorrows.find(t => t.id === returnForm.transactionId);
  ...
  const handleBorrowSubmit = (e) => {
    ...
    const eq = mockEquipment.find(e => e.id === borrowForm.equipmentId);
  ...
  const handleReturnSubmit = (e) => {
    ...
    const txn = myActiveBorrows.find(t => t.id === returnForm.transactionId);
          ...
          <div className="lg:col-span-2 bg-white rounded-2xl shadow-sm border border-gray-200 p-8">
            ...
            {myActiveBorrows.length === 0 ? (
              ...
              <form onSubmit={handleReturnSubmit} className="space-y-5">
                ...
                {selectedTxn && (() => {
                  const eq = mockEquipment.find(e => e.id === selectedTxn.equipmentId);
